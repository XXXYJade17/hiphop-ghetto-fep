        ...
        <div class="auth-container flex gap-8 items-center">
          ...
          <div class="auth-illustration w-1/2 hidden lg:block">
            <div class="rounded-2xl overflow-hidden relative">
              <img
                alt="Hip hop culture illustration"
                class="w-full h-full object-cover rounded-2xl"
                src="https://www.baidu.com/img/flexible/logo/pc/result.png"
              />
              <div
                class="absolute inset-0 bg-gradient-to-tr from-dark to-transparent opacity-70"
              ></div>
              <div class="absolute bottom-8 left-8 right-8">
                <h2 class="text-3xl font-bold text-white mb-4 font-['Exo 2']">加入嘻哈社区</h2>
                <p class="text-gray-custom mb-6">
                  与全球说唱爱好者一起分享、发现和创作最棒的嘻哈音乐
                </p>
                <div class="flex items-center space-x-4">
                  ...
                  <p class="text-sm text-gray-dark">
                    已有 <span class="text-primary">10,000+</span> 嘻哈爱好者加入
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="card-bg p-8">
              ...
              <div class="mb-8">
                <div class="flex relative">
                  ...
                  <div
                    class="tab-indicator absolute bottom-0"
...
// 引入认证逻辑
const { Validator, showToast, registerUser, loginUser } = useAuth()

// 更新登录表单
const updateLoginForm = (newForm) => {
  loginForm.value = newForm
}

// 更新注册表单
const updateRegisterForm = (newForm) => {
  registerForm.value = newForm
}
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (result.success) {
      showToast(result.message, true, (msg, type) => {
        toastMessage.value = msg
        toastType.value = type
        toastShow.value = true
      })
      ...
      showToast(result.message, false, (msg, type) => {
        toastMessage.value = msg
        toastType.value = type
        toastShow.value = true
      })
...
const handleRegister = async () => {
  ...
  try {
    const result = await registerUser({
      ...
      phone: registerForm.value.phone || undefined, // 如果为空则不传递该字段
      email: registerForm.value.email || undefined, // 如果为空则不传递该字段
